...
func (r *Resolver) lookup(ctx context.Context, req *dns.Msg) (*dns.Msg, error) {
	...
	for {
		...
		if err != nil {
			log.Printf("Error during recursive lookup for %s: %v", q.Name, err) // Added logging
		...
		if len(nextNS) > 0 {
			...
			for _, ns := range nextNS {
				...
				if cachedNsMsg, found, _ := r.cache.Get(cache.Key(nsReq.Question[0])); found {
					...
					if err != nil {
						log.Printf("Failed to resolve NS %s during recursive lookup: %v", ns, err) // Added logging
						continue
					}
...
func (r *Resolver) query(ctx context.Context, nsAddrs []string, req *dns.Msg) (*dns.Msg, error) {
	...
	for _, addr := range nsAddrs {
		...
		if err != nil {
			log.Printf("Error querying %s for %s: %v", addr, req.Question[0].Name, err) // Added logging
			continue
		}
		return resp, nil
	}
	log.Printf("All nameservers failed to respond for %s", req.Question[0].Name) // Added logging
